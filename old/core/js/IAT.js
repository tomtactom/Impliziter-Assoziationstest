...
function checkDemographics() {
  ...
  if (error == false) {
    ...
    demos += education + '\n';
    $.post("core/writeFile.php", {
      'subject': subject,
      'src': "survey",
      'data': demos
    }, function() {
      location.href = 'instruct2.php?sub=' + sub;
    });
  } else {
    $('#error-1').html(errmsg);
  }
  ...
  for (i = 0; i < roundArray.length; i++) {
    for (j = 0; j < roundArray[i].length; j++) {
      ...
      $.post("core/fileManager.php", {
        ...
        'datai': i,
        'dataj': j,
        'category': roundArray[i][j].category,
        'catindex': roundArray[i][j].catIndex,
        'errors': roundArray[i][j].errors,
        'mseconds': (roundArray[i][j].endtime - roundArray[i][j].starttime)
      });
    }
  }
